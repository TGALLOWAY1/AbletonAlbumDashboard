...
import { isMissingColumn, MIGRATION_0027_MISSING_MESSAGE } from "@/lib/migration-errors";
import {
 ...
 * task that belongs to no track (migration 0027). Everything else — the
...
export async function addTrackTodo(input: {
  ...
  const { error } = await supabase.from("actions").insert({
    ...
    // track to join, so they carry it (migration 0027); writing it on both
    // keeps one row shape.
    owner_id: OWNER_ID,
    description: parsed.description,
  });
  if (error) {
    if (isMissingColumn(error)) throw new Error(MIGRATION_0027_MISSING_MESSAGE);
    throw error;
  }
